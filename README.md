---
title: "human-skills"
type: home
tags:
  - home
status: active
verified: 2026-08-28
---

# human-skills

Catalogue vivant de **certifications, examens, credentials et qualifications professionnelles**, classés par coût, domaine, type de preuve et **TCO réel**.

> **Dernière revue globale : 28 août 2026**

## Objectif

Répondre à quatre questions :

1. **Qu'est-ce que je peux passer gratuitement ?**
2. **Quelles certifications valent leur coût selon mon budget ?**
3. **Quel est le vrai TCO du credential, pas seulement le prix du voucher ?**
4. **Dans quel ordre les passer pour construire un profil cohérent ?**

Le dépôt couvre désormais IT et hors IT : cloud, infrastructure, stockage, backup, cyber, réseau, Wi-Fi, AI infrastructure, data, architecture, observabilité, automation, projet, leadership, **entrepreneuriat / création d'entreprise / startup**, finance, actuariat, assurance, audit, gouvernance, AML/compliance, réglementation AMF, ESG, carbone, green building, pharma/GxP, supply chain, change management, datacenter, facilities, énergie, QHSE, OT/ICS, continuité d'activité et autres domaines.

---

# Catalogue

## Gratuit

- [`certifications/free-it.md`](certifications/free-it.md) — Oracle, Commvault, Neo4j, KNIME, Microsoft Applied Skills, Qualys, Calico, etc.
- [`certifications/free-non-it.md`](certifications/free-non-it.md) — anglais, management, projet, RGPD, Lean/Six Sigma, business, marketing, etc.

## Général / jusqu'à environ 500 €

- [`certifications/paid-under-500.md`](certifications/paid-under-500.md) — catalogue général.
- [`certifications/business-finance-under-500.md`](certifications/business-finance-under-500.md) — PMI, Scrum.org, ISACA, TOGAF, Lean Six Sigma, CFA Investment Foundations, langues, business.

## Entrepreneuriat, startup & création d'entreprise

- [`certifications/entrepreneurship-startup-business-creation-2026.md`](certifications/entrepreneurship-startup-business-creation-2026.md) — gratuit → >5 000 € : IBM SkillsBuild, HP LIFE, OpenLearn, Certiport ESB, CCI France, BGE, CréActifs, Cnam, NCFE/SFEDI, HBS Online et HEC Paris.

## Infrastructure, cloud, réseau & plateformes

- [`certifications/tools-platforms-under-500.md`](certifications/tools-platforms-under-500.md) — Datadog, Confluent, dbt, UiPath, HPE, IBM, ServiceNow, Elastic, MOS, Adobe, Autodesk, etc.
- [`certifications/storage-data-protection-under-500.md`](certifications/storage-data-protection-under-500.md) — NetApp, Dell, Pure Storage, Hitachi Vantara, Nutanix, VAST, Cohesity, NetBackup/Veritas, Huawei.
- [`certifications/networking-wireless-ai-infra.md`](certifications/networking-wireless-ai-infra.md) — Juniper, CWNP, Nokia, MEF, NVIDIA AI Infrastructure / Networking.
- [`certifications/finops-veeam-over-500.md`](certifications/finops-veeam-over-500.md) — FinOps Professional et nouveau parcours Veeam VMCE+/VMCSE 2026.
- [`certifications/datacenter-facilities-over-500.md`](certifications/datacenter-facilities-over-500.md) — Uptime ATD/ATS/AOS/ATP, BICSI DCDC/RCDD, Schneider DCCA à chiffrer.
- [`certifications/facilities-energy-qhse.md`](certifications/facilities-energy-qhse.md) — IFMA, AEE CEM/CEA/CMVP, IOSH, ISO 45001.

## Cybersécurité

- [`certifications/practical-cyber-under-500.md`](certifications/practical-cyber-under-500.md) — HTB, TCM, Altered Security, Zero-Point, CREST Practitioner, TryHackMe, BTL1, INE, BSCP.
- [`certifications/cyber-premium-over-500.md`](certifications/cyber-premium-over-500.md) — CREST Registered/Certified, OffSec, OSCE3/OSEE, GIAC/SANS, Altered Security bundles, Zero-Point bundles.
- [`certifications/industrial-ot-automation.md`](certifications/industrial-ot-automation.md) — ISA CAP/CCST, ISA/IEC 62443, SIS / functional safety, OT/ICS.

## ISO, GRC, continuité & transformation

- [`certifications/iso-grc-europe-over-500.md`](certifications/iso-grc-europe-over-500.md) — AFNOR, BSI, PECB, CQI/IRCA, ISO 27001, 42001 et autres normes.
- [`certifications/business-continuity-crisis.md`](certifications/business-continuity-crisis.md) — BCI CBCI, DRI ABCP/CBCP/CFCP, resilience.
- [`certifications/management-transformation-over-500.md`](certifications/management-transformation-over-500.md) — Prosci, SABSA, IIBA CBAP, DAMA CDMP avancé, HRCI, SAFe, procurement.
- [`certifications/audit-finance-project-over-500.md`](certifications/audit-finance-project-over-500.md) — IIA CIA/CRMA, IMA CMA, ACCA, IPMA France, SMaP Change, ACMP CCMP, APMG.
- [`certifications/lean-it-lean-management-2026.md`](certifications/lean-it-lean-management-2026.md) — Lean IT (EXIN, retrait PeopleCert), IASSC, SME/AME/Shingo, IISE, Kaizen Institute.

## Finance, risk, assurance & actuariat

- [`certifications/paid-over-500.md`](certifications/paid-over-500.md) — catalogue général premium : ISC2, ISACA, IAPP, GIAC/SANS, Cisco, Red Hat, PECB, PeopleCert, PMI, GARP, CFA, ASQ, ASCM, TOGAF, etc.
- [`certifications/finance-risk-fraud-over-500.md`](certifications/finance-risk-fraud-over-500.md) — CAIA, PRMIA, CFE, CTP, credit/counterparty risk.
- [`certifications/compliance-aml-fpa-over-500.md`](certifications/compliance-aml-fpa-over-500.md) — CAMS, CAFS, CGSS, Advanced CAMS, ICA AML/GRC/Financial Crime, AFP FPAC.
- [`certifications/france-amf-regulatory.md`](certifications/france-amf-regulatory.md) — AMF généraliste, Finance Durable, RCCI, RCSI.
- [`certifications/actuarial-accounting-insurance.md`](certifications/actuarial-accounting-insurance.md) — SOA ASA/FSA, IFoA Associate/Fellow, CIMA/CGMA, LOMA FLMI.
- [`certifications/insurance-risk-designations.md`](certifications/insurance-risk-designations.md) — CAS ACAS/FCAS, CPCU, ARM, ARe, CII Diploma/ACII/FCII.

## ESG, environnement & bâtiment durable

- [`certifications/esg-sustainability-carbon.md`](certifications/esg-sustainability-carbon.md) — CFA Sustainable Investing, IFRS FSA, GHGMI, IEMA, GARP SCR.
- [`certifications/green-building-sustainability.md`](certifications/green-building-sustainability.md) — LEED, WELL, BREEAM, GRI.

## Pharma, qualité & réglementaire

- [`certifications/pharma-clinical-regulatory.md`](certifications/pharma-clinical-regulatory.md) — ASQ CPGP, RAPS RAC/RCC/FRA, ACRP, SOCRA.

## Supply chain, achats & gouvernance

- [`certifications/supply-chain-procurement-over-500.md`](certifications/supply-chain-procurement-over-500.md) — ASCM/APICS CPIM/CSCP/CLTD/CTSC, CSCMP SCPro, ISM CPSM/CPSD, CIPS/MCIPS.
- [`certifications/governance-board-over-500.md`](certifications/governance-board-over-500.md) — NACD.DC, IoD Chartered Director, Chartered Governance Institute.

## Roadmap

- [`roadmap.md`](roadmap.md) — logique de construction du profil et ordre de passage recommandé.

## Recherche et sources

- [`sources/community-repositories.md`](sources/community-repositories.md) — anciens et nouveaux dépôts GitHub utilisés comme **radars**, avec leur fiabilité 2026.
- [`research/over-500-watchlist.md`](research/over-500-watchlist.md) — uniquement ce qui reste réellement à consolider.

## Structure du dépôt

- [`OBSIDIAN.md`](OBSIDIAN.md) — conventions du vault (frontmatter, tags `tier/…`/`domain/…`, gabarit de nouvelle fiche) pour ouvrir ce dépôt comme coffre Obsidian.

---

# Tranches de prix

```text
FREE
FREE-CONDITIONAL
< 100 €
100–250 €
250–500 €
500–1 000 €
1 000–2 500 €
2 500–5 000 €
5 000–10 000 €
> 10 000 €
```

À partir de 500 €, le dépôt conserve autant que possible trois mesures :

```text
EXAM PRICE
    Voucher / examen.

FIRST-CYCLE TCO
    Coût minimal réaliste pour obtenir et activer le credential.

LONG-TERM TCO
    Maintenance, renouvellement, membership, CPD/CPE/PDU et obligations sur plusieurs années.
```

---

# Cas qui montrent pourquoi le TCO est indispensable

```text
Commvault Engineer / Expert
ILT 3 000–5 000 $
mais standalone exam curriculum = 0 $
→ formation facultative ≠ coût obligatoire
```

```text
Juniper JNCIP
prix catalogue 400 $
Open Learning + assessment éligible → voucher -75 %
= 100 $
```

```text
Veeam VMCE+
exam price
+ 3 formations obligatoires
→ parcours premium même si le voucher est raisonnable
```

```text
AEE CEM
certification/exam 500 $
+ approved training obligatoire ~1,3–2,4 k$
→ TCO réel ~1,8–2,9 k$
```

```text
ISA/IEC 62443 Expert
4 certificats obligatoires
→ minimum ~6 912 $ membre / 8 640 $ non-membre en self-paced
```

```text
OSCE3
pas un examen unique
= OSEP + OSWE + OSED
→ plusieurs milliers de dollars depuis zéro
```

```text
SOA ASA
une succession d'examens + modules + VEE + professionalism
→ ~7 553 $ de frais officiels standard avant préparation / retakes
```

```text
Uptime ATP
ATP 4 985 $
+ ATD ou ATS préalable 4 985 $
→ minimum 9 970 $ depuis zéro
```

---

# Niveaux de preuve

## Certification professionnelle

Examen formel, surveillé, pratique ou contrôlé.

Exemples : AWS, Cisco, CNCF, Red Hat, HashiCorp, ISC2, ISACA, CREST, HTB, TCM, Commvault.

## Qualification professionnelle

Programme plus large avec plusieurs examens / assessments, crédits, expérience ou statut professionnel.

Exemples : IFoA Fellow, SOA ASA/FSA, CIMA/CGMA, CII ACII, IoD, Chartered Governance Institute, CFA, ACCA.

## Certification de personne

Exemple : certaines certifications AFNOR ICA, distinctes d'une simple formation ISO.

## Carte / exigence réglementaire

Exemples : RCCI/RCSI AMF et examens réglementaires associés.

## Credential appliqué

Évaluation pratique éditeur ne suivant pas nécessairement le modèle proctoré classique.

Exemple : Microsoft Applied Skills.

## Digital credential / badge

Évaluation ou parcours vérifiable utile en complément.

## Attestation / certificat de formation

Prouve qu'un parcours a été suivi, sans être assimilé automatiquement à une certification professionnelle.

---

# Méthode de vérification

```text
Découverte / ancien dépôt GitHub
   ↓
Source officielle actuelle
   ↓
Credential encore actif ?
   ↓
Prix examen / assessment / units
   ↓
Prix régional / TVA / devise
   ↓
Prérequis
   ↓
Formation obligatoire ?
   ↓
Credential préalable obligatoire ?
   ↓
Licence / abonnement obligatoire ?
   ↓
Application / membership ?
   ↓
Maintenance / CPE / PDU / CPD ?
   ↓
Retakes / déplacements ?
   ↓
First-cycle TCO
   ↓
Long-term TCO
   ↓
Date de vérification
```

Pour les conversions USD/EUR de la revue du 28 août 2026, référence indicative utilisée :

```text
27 août 2026
1 EUR = 1,1645 USD
1 USD ≈ 0,8587 EUR
```

Les conversions ne remplacent jamais le prix final du checkout.

---

# Dépôts communautaires retrouvés

Radars utilisés pour retrouver d'anciennes gratuités et niches :

- `munchy-bytes/FreeDevCertifications` ;
- `ArslanYM/Free-Certifications` ;
- `PanXProject/awesome-certificates` ;
- `Troy-LL/The-Free-Credential-Index` ;
- `orgito1015/free-cybersecurity-certifications` ;
- `surajbhan-3/Free-IT-Certification-and-badges_list`.

Ils sont utiles pour la **découverte**, jamais comme source de prix 2026 sans revérification.

---

# Quelques rapports valeur / prix remarquables en 2026

| Credential | Prix indicatif |
|---|---:|
| Commvault Professional / Engineer / Expert | **0 €** |
| Neo4j Certified Professional | **0 €** |
| KNIME L1 | **0 €** |
| CNIL Atelier RGPD | **0 €** |
| Juniper JNCIA via Open Learning | **50 $** |
| HashiCorp Terraform Associate | **70,50 $** |
| AMF généraliste / Finance Durable Cnam exam-only | **80 € chacun** |
| GitHub Certifications | **99 $ global / régionalisation** |
| Juniper JNCIP via voucher éligible | **100 $** |
| Datadog | **100 $** |
| NVIDIA AI Infrastructure Associate | **125 $** |
| AWS Solutions Architect Associate | **128 €** |
| Confluent Kafka | **150 $** |
| NetApp exam | **200 $** |
| Cohesity Protection / Security | **200 $** |
| Pure Storage Associate | **200 $** |
| Scrum.org PSM I | **200 $** |
| Dell Proven Professional | **230 $** |
| Altered Security CRTP | **249 $** |
| LEED Green Associate | **250 $** |
| CREST Practitioner | **275 £ + taxes** |
| Altered Security CRTE | **299 $ / 30 jours** |
| WELL AP | **299 $** |
| Cisco CCNA | **300 $** |
| VAST VCP-AD1 | **300 $** |
| TryHackMe PT1 / SAL1 | **301 €** |
| CFA Investment Foundations | **350 $** |
| Zero-Point RTO | **365 £** |
| NVIDIA AI Infrastructure / Networking Professional | **400 $** |
| GHGMI Certificate of Proficiency typique | **435 $** |
| LFCS / CKA / CKS | **445 $** |
| ASQ CPGP membre | **450 $** |
| PMI-ACP | **495 $ non-membre** |
| TCM PNPT / PSAP | **499 $** |
| BICSI DCDC / RCDD | **725 $ exam** |
| CISSP | **719,04 € + maintenance** |
| CFA Sustainable Investing | **890 $** |
| DRI CBCP minimal sans cours | **900 $ + 225 $/an** |
| BCI CBCI self-study | **1 280 £** |
| Juniper JNCIE | **1 600 $** |
| FinOps Certified Professional | **~1 999 $ à confirmer au checkout** |
| GARP FRM complet standard depuis zéro | **~2 000 $ + taxes** |
| CGI Advanced Certificate Corporate Governance | **2 050 £** |
| ASCM CSCP bundle | **2 620–3 650 $** |
| BSI ISO 27001 Lead Auditor | **2 795 € + TVA** |
| Prosci Change Management | **3 386 € online / 3 810 € présentiel HT** |
| CPCU — 8 Recommended Packages | **~3 352 $** |
| NACD Directorship Certification package | **3 995 $** |
| LOMA FLMI — employer member pricing | **4 250 $** |
| IFoA Associate — 10 examens Core | **4 637 £ + membership/PPD** |
| Nokia SRA self-study full bundle | **4 500 €** |
| OSCE3 via 3 OffSec Course+Cert | **~5 247 $** |
| SOA ASA standard | **~7 553 $ avant prep/retakes** |
| LOMA FLMI non-member pricing | **8 500 $** |
| ISA/IEC 62443 Expert self-paced | **~6 912–8 640 $ + éventuel membership** |
| Uptime ATD/ATS → ATP | **9 970 $ minimum** |
| IoD Certificate in Company Direction | **12 250–15 950 £ HT** |

---

# Maintenance des données

Pour chaque entrée, conserver si possible :

- nom exact ;
- organisme ;
- domaine ;
- niveau de preuve ;
- prix officiel ;
- monnaie ;
- conversion EUR indicative ;
- TVA / taxes ;
- prérequis ;
- formation obligatoire/facultative ;
- credentials préalables ;
- licence / abonnement ;
- tentatives ;
- expiration / renouvellement ;
- application ;
- membership ;
- maintenance ;
- CPE / PDU / CPD ;
- retake ;
- déplacement ;
- first-cycle TCO ;
- long-term TCO ;
- source officielle ;
- date de vérification ;
- statut : actif / conditionnel / promotion / deprecated / research.

Le but final est une **cartographie internationale du TCO des certifications et qualifications professionnelles**, pas une simple collection de liens.
