---
title: "IT Certification Index — human-skills"
type: moc
tags:
  - moc
  - domain/general-it
status: active
verified: 2026-08-28
---

# IT Certification Index — human-skills

> Index **IT-first** : certifications infrastructure, cloud, virtualisation, Linux, réseau, cyber, stockage, data, platform engineering, middleware, EUC, ITSM et AI infrastructure.
>
> Revue : **28 août 2026**.

## Méthode

Le dépôt distingue toujours :

```text
exam_fee
mandatory_training
mandatory_credentials
first_cycle_tco
renewal_tco
```

Un examen à 200 $ avec 3 000 € de formation obligatoire n'est donc **pas** classé comme une certification à 200 €. Les prix sont des références publiques : vérifier taxes, devise France/EMEA, version active et prérequis avant achat.

---

# 1. Gratuit / presque gratuit

- [`certifications/free-it.md`](certifications/free-it.md)
- [`certifications/middleware-api-integration-2026.md`](certifications/middleware-api-integration-2026.md)
- [`certifications/identity-iam-pam-under-500.md`](certifications/identity-iam-pam-under-500.md)
- [`certifications/observability-vendor-low-cost-2026.md`](certifications/observability-vendor-low-cost-2026.md)

```text
0 €      OCI Architect Associate
0 €      Oracle Foundations / Agentic AI Foundations
0 €      Commvault Professional / Engineer / Expert standalone
0 €      Qualys VMDR / Policy Audit / CSAM
0 €      Microsoft Applied Skills ciblés
0 $      Neo4j Professional / GDS
0 $      QRadar SIEM Foundation Badge
0 $      New Relic Verified Foundation
0 $      SailPoint Knowledge Credentials
0 $      Boomi — nombreux credentials
0 $      Solace Developer / EDA / Agent Mesh
```

---

# 2. Public cloud & multicloud

- [`certifications/public-cloud-multicloud-under-500.md`](certifications/public-cloud-multicloud-under-500.md)

```text
OCI Architect Associate          0 €
Azure role-based exam          126 €
AWS Associate                  128 €
Google ACE                     125 $
Google Professional            200 $
AWS Professional/Specialty     256 €
```

---

# 3. Virtualisation / private cloud / open infrastructure

- [`certifications/virtualization-private-cloud-under-500.md`](certifications/virtualization-private-cloud-under-500.md)
- [`certifications/open-source-private-cloud-2026.md`](certifications/open-source-private-cloud-2026.md)
- [`certifications/hpe-certification-pricing-ai-private-cloud-2026.md`](certifications/hpe-certification-pricing-ai-private-cloud-2026.md)

```text
CloudStack by ShapeBlue          100 $
Nutanix NCP                     ~199 $
VMware VCP                       250 $
Nutanix NCM                      300 $ + prerequisite
OpenStack COA                    400 $
OpenNebula Certified Admin       899 €
```

**SUSE Virtualization** : VIRT201v1.8 est sorti en 2026 et prépare à une SCA in SUSE Virtualization ; page/prix d'examen encore insuffisamment exposés publiquement lors de la revue.

**Proxmox** : courses + Certificate of Completion, pas de certification proctored standalone confirmée.

---

# 4. Windows Server / Azure Local

- [`certifications/microsoft-windows-infrastructure-2026.md`](certifications/microsoft-windows-infrastructure-2026.md)
- [`certifications/microsoft-azure-local-hci-2026.md`](certifications/microsoft-azure-local-hci-2026.md)

```text
Azure Local Accreditation 2025       0 €  achievement Learn
Windows Server 2025 Accreditation    0 €  achievement Learn
AZ-802                              126 €  certification role-based
```

AZ-800 + AZ-801 sont retirés le **30 septembre 2026**.

---

# 5. Enterprise Linux / Red Hat / SUSE / Canonical

- [`certifications/redhat-openshift-linux-tco.md`](certifications/redhat-openshift-linux-tco.md)
- [`certifications/redhat-framework-europe-2026.md`](certifications/redhat-framework-europe-2026.md)
- [`certifications/enterprise-linux-suse-canonical-2026.md`](certifications/enterprise-linux-suse-canonical-2026.md)

```text
SUSE AI Deployment Specialist       99 $
SLES 16 Administrator              149 $
RKE2 Administrator                 149 $
RKE2 Deployment Specialist         149 $
Longhorn Administrator             149 $
SUSE Edge                          149 $
SUSE Observability                 149 $
SUSE Security / NeuVector          149 $
SLE High Availability Engineer     195 $
SLES for SAP Engineer              298 $  (2 examens)
```

Canonical Academy est actif mais le prix public du shop est derrière Ubuntu SSO.

Red Hat a refondu son framework le **11 mai 2026**. Exemple EMEA observé : environ **530 € HT** pour un voucher KIOSK ; ne pas traduire automatiquement « 500 $ US » en « ≤500 € France ».

---

# 6. Kubernetes & platform engineering

- [`certifications/cloud-native-platform-engineering-under-500.md`](certifications/cloud-native-platform-engineering-under-500.md)

```text
SUSE RKE2              149 $
Prometheus PCA          250 $
Cilium CCA              250 $
OpenTelemetry OTCA      250 $
CKA / CKAD / CKS        445 $ selon examen
CNPE                    445 $
LFCS                    445 $
```

---

# 7. IaC / IAM / PAM / DevSecOps

- [`certifications/iam-devsecops-automation-under-500.md`](certifications/iam-devsecops-automation-under-500.md)
- [`certifications/identity-iam-pam-under-500.md`](certifications/identity-iam-pam-under-500.md)
- [`certifications/cyberark-certification-status-2026.md`](certifications/cyberark-certification-status-2026.md)
- [`certifications/beyondtrust-certification-tco-2026.md`](certifications/beyondtrust-certification-tco-2026.md)

```text
SailPoint Knowledge Credentials     0 $
Terraform Associate             70,50 $
Vault Associate                 70,50 $
GitHub certifications              99 $
Okta Professional                  250 $
Vault Operations Professional      295 $
Ping Identity Professional         365 €
SailPoint Professional          400–500 $
```

CyberArk : examens live, **prix 2026 non public** ; ne plus utiliser l'ancien « ~200 $ ».

BeyondTrust : ILT obligatoire, validité 2 ans, recertification par nouveau parcours de formation ; TCO employer/customer-funded.

---

# 8. Networking / DDI / automation / AI fabrics

- [`certifications/networking-wireless-ai-infra.md`](certifications/networking-wireless-ai-infra.md)
- [`certifications/network-datacenter-advanced-under-500.md`](certifications/network-datacenter-advanced-under-500.md)
- [`certifications/network-automation-under-500-2026.md`](certifications/network-automation-under-500-2026.md)
- [`certifications/open-networking-sonic-ai-fabrics-2026.md`](certifications/open-networking-sonic-ai-fabrics-2026.md)
- [`certifications/arista-academy-certification-2026.md`](certifications/arista-academy-certification-2026.md)

```text
Infoblox Associate / Operator            29 $
JNCIA / JNCIA-DevOps Open Learning       50 $
Infoblox Professional / Administrator    69 $
Infoblox Expert                           99 $
Nokia NSP Network Automation            125 $
NVIDIA NCA-AIIO                          125 $
Arista Associate / Specialist            295 $
Cisco DevNet Associate                  300 $
Extreme Administrator               325 € / 375 $
Arista Professional                     395 $
NVIDIA NCP AI tracks                    400 $
Extreme Professional                    495 $ régional
NVIDIA NCP-AIO                           500 $
Arista Expert                           1995 $
```

SONiC : pas de certification professionnelle standalone officielle confirmée.

---

# 9. Network security / ADC / SASE

- [`certifications/network-security-adc-sase-under-500.md`](certifications/network-security-adc-sase-under-500.md)

Couvre F5, Palo Alto, Fortinet, Check Point, Zscaler, Cloudflare, Akamai, Netskope, Cato.

---

# 10. Storage & data protection

- [`certifications/storage-data-protection-under-500.md`](certifications/storage-data-protection-under-500.md)
- [`certifications/open-source-storage-ceph-minio-2026.md`](certifications/open-source-storage-ceph-minio-2026.md)
- [`certifications/rubrik-certification-status-2026.md`](certifications/rubrik-certification-status-2026.md)
- [`certifications/finops-veeam-over-500.md`](certifications/finops-veeam-over-500.md)

```text
Commvault Engineer / Expert             0 $ selon curricula gratuits
Longhorn                               149 $
Nutanix NCP                            199 $
NetApp                                 200 $
Cohesity / NetBackup                   200 $
Pure Associate                         200 $
Hitachi HCE                            225 $
Dell Proven Professional               230 $
MinIO Certified Administrator          300 $
Portworx Enterprise Professional    300–400 $ selon storefront/doc
VAST VCP-AD1                           300 $
Pure Expert                            400 $
```

Ceph : EX260 Red Hat reste un credential identifiable, mais coût France/EMEA typiquement hors low-cost. Pas de certification Ceph Foundation/LF standalone confirmée.

Rubrik : préparation RCSA gratuite, final exam entitlement-gated, prix public non exposé.

Veeam : VMCE+ exige VBR + ONE + Recovery Orchestrator ; exemple France **3 440 € HT pour le seul cours VBR v13**.

---

# 11. Databases & data platforms

- [`certifications/data-database-platforms-under-500.md`](certifications/data-database-platforms-under-500.md)

```text
MongoDB                  150 $
Confluent Kafka          150 $
SnowPro Core             175 $
Databricks Associate     200 $
SnowPro Advanced         375 $
```

Inclut PostgreSQL/EDB, Redis, Elastic, Oracle Database et MySQL.

---

# 12. Messaging / event streaming / integration

- [`certifications/middleware-api-integration-2026.md`](certifications/middleware-api-integration-2026.md)
- [`certifications/enterprise-messaging-streaming-2026.md`](certifications/enterprise-messaging-streaming-2026.md)

```text
0 $      Boomi certifications
0 $      Solace Developer / EDA / Agent Mesh
150 $    Confluent Kafka
200 $    MuleSoft Hyperautomation Developer
295 $    Pulsar Certified Developer — registrations currently closed
400 $    MuleSoft Platform Architect
```

RabbitMQ / NATS : aucune certification professionnelle officielle actuelle confirmée.

---

# 13. API management

- [`certifications/api-management-specialists-2026.md`](certifications/api-management-specialists-2026.md)

```text
50 $     WSO2 API Manager Practitioner
100 $    WSO2 API Manager Developer
275 $    Kong Gateway Certified Associate
275 $    Kong Konnect Certified Associate
450 $    WSO2 Developer → Expert minimal path
500 $    WSO2 Practitioner → Expert complet
```

Kong 2026 : **275 $**, passing 75 %, formation non obligatoire, validité 3 ans, retake **49 $** après 2 semaines.

Tyk = course-completion certificates ; Apigee = skill badges ; Gravitee/Axway = programmes live mais prix non public clair.

---

# 14. Observability / SRE / DevOps

- [`certifications/observability-sre-devops-under-500.md`](certifications/observability-sre-devops-under-500.md)
- [`certifications/observability-vendor-low-cost-2026.md`](certifications/observability-vendor-low-cost-2026.md)

```text
New Relic Verified Foundation     0 $
Datadog                          100 $
Sumo Logic Fundamentals          100 $
Splunk                           130 $
Sumo Logic Advanced              150 $
Google Cloud DevOps              200 $
Prometheus / OpenTelemetry       250 $
AWS DevOps Pro                   256 €
Elastic                       400–500 $
```

Grafana Labs : pas de certification proctored officielle clairement tarifée confirmée.

---

# 15. CI/CD & DevOps tooling

- [`certifications/cicd-devops-tooling-2026.md`](certifications/cicd-devops-tooling-2026.md)

GitLab : Git Associate, CI/CD Associate, Agile Project Management Associate, Security Specialist, GitLab Duo AI Associate ; préparation gratuite, prix actuel checkout-required.

CloudBees/Jenkins CJE : documents historiques officiels, mais live registration/prix 2026 non confirmés → **legacy/status-to-confirm**.

---

# 16. EUC / VDI / endpoint

- [`certifications/euc-vdi-endpoint-under-500-2026.md`](certifications/euc-vdi-endpoint-under-500-2026.md)

```text
Jamf Certified Associate               100 $
Citrix Associate                       200 $
Omnissa exam                           250 $
Citrix Professional / Expert           300 $
Parallels RAS TPA France             1750 € HT  exemple ILT + exam
```

---

# 17. Server / compute OEM

- [`certifications/server-compute-oem-dell-lenovo-supermicro-2026.md`](certifications/server-compute-oem-dell-lenovo-supermicro-2026.md)
- [`certifications/hpe-certification-pricing-ai-private-cloud-2026.md`](certifications/hpe-certification-pricing-ai-private-cloud-2026.md)

```text
HPE3 exam                               65 $
Dell Skill Certification partner       115 $ si éligible
HPE2                                   140 $
Dell Proven Professional retail        230 $
HPE0 / HPE6                            260 $
HPE7                                   350 $
HPE ATP AI Solutions                   390 $ combinaison d'examens
Lenovo Data Center Technical             ?  partner/employee centric
Supermicro professional cert             ?  portal-centric
```

Dell PowerEdge 2026 : Foundations, Operate et PowerEdge XE AI/HPC sont des credentials actuels. PowerEdge XE Install a été renouvelé en juillet 2026.

---

# 18. AI infrastructure / GPU / HPC

- [`certifications/ai-engineering-mlops-agents-under-500.md`](certifications/ai-engineering-mlops-agents-under-500.md)
- [`certifications/ai-infrastructure-gpu-hpc-2026.md`](certifications/ai-infrastructure-gpu-hpc-2026.md)

```text
NVIDIA NCA-AIIO                    125 $
NVIDIA NCP-AIN / AII / ARI        400 $
NVIDIA NCP-AIO                     500 $
AMD ROCm Associate                   ?  programme lancé juillet 2026
```

AMD ROCm Professional est annoncé plus tard en 2026, Expert début 2027. Prix public non exposé lors de la revue.

---

# 19. ITSM / enterprise tooling

- [`certifications/itsm-enterprise-tooling-servicenow-atlassian-bmc-2026.md`](certifications/itsm-enterprise-tooling-servicenow-atlassian-bmc-2026.md)
- [`certifications/lean-it-lean-management-2026.md`](certifications/lean-it-lean-management-2026.md) — Lean IT (EXIN, retrait PeopleCert fin 2026), IASSC, SME/AME/Shingo, IISE, Kaizen Institute.

```text
Atlassian Certificates                       0 $  unproctored
Atlassian Certified Associate              100 $
ServiceNow CSA/CAD/CPOA retake             150 $
ServiceNow CIS retake                      225 $
Atlassian Certified Professional           249 $
ServiceNow CSA initial attempt            ~300 $ + taxes
BMC Helix Certified Professional              ?  checkout/contact
```

ServiceNow impose aussi maintenance/delta/CMP dans le cycle de vie ; ne pas regarder seulement le premier exam fee.

---

# 20. Cybersécurité pratique

- [`certifications/practical-cyber-under-500.md`](certifications/practical-cyber-under-500.md)
- [`certifications/cyber-premium-over-500.md`](certifications/cyber-premium-over-500.md)

Couvre HTB, TCM, TryHackMe, Security Blue Team, Altered Security, Zero-Point, CREST, INE, PortSwigger, OffSec, GIAC/SANS.

---

# 21. Enterprise architecture / governance / GRC

- [`certifications/paid-over-500.md`](certifications/paid-over-500.md)
- [`certifications/iso-grc-europe-over-500.md`](certifications/iso-grc-europe-over-500.md)
- [`certifications/ai-governance-risk-safety.md`](certifications/ai-governance-risk-safety.md)
- [`certifications/bcs-chartered-institute-it-2026.md`](certifications/bcs-chartered-institute-it-2026.md)

TOGAF, CISSP/CCSP, ISACA, IAPP, ISO 27001/42001, SABSA, AI governance, BCS (architecture, sécurité info, data protection, business analysis, ISTQB).

---

# 22. Mainframe / UNIX / enterprise software

- [`certifications/mainframe-enterprise-software.md`](certifications/mainframe-enterprise-software.md)
- [`certifications/ibm-enterprise-security-ai-under-500.md`](certifications/ibm-enterprise-security-ai-under-500.md)

```text
QRadar Foundation Badge             0 $
IBM AIX Administrator             100 $
IBM z/OS Administrator Pro        200 $
IBM Db2 13 z/OS DBA               200 $
IBM MQ 9.4 Administrator          200 $
IBM QRadar / Guardium             200 $
SAP one attempt                   230 $
```

RACF/CICS/IMS restent en watchlist credential-live.

---

# 23. OT / ICS / Edge

- [`certifications/industrial-ot-automation.md`](certifications/industrial-ot-automation.md)
- [`certifications/automation-semiconductor.md`](certifications/automation-semiconductor.md)

ISA CAP/CCST, IEC 62443, functional safety, automation industrielle, semiconductor.

SUSE Edge Deployment Specialist : **149 $**, cours non obligatoire.

---

# 24. Datacenter / facilities

- [`certifications/datacenter-facilities-over-500.md`](certifications/datacenter-facilities-over-500.md)
- [`certifications/datacenter-france-titre-professionnel-2026.md`](certifications/datacenter-france-titre-professionnel-2026.md)
- [`certifications/facilities-energy-qhse.md`](certifications/facilities-energy-qhse.md)

Uptime Institute, BICSI, Schneider Electric, EPI/EXIN, titre professionnel France (AFPA × Equinix), energy/facilities/QHSE.

---

# 25. FinOps / cloud economics

- [`certifications/finops-veeam-over-500.md`](certifications/finops-veeam-over-500.md)

Practitioner/Engineer peuvent rester autour ou sous 500 $ selon formule. Professional impose plusieurs credentials préalables et un TCO supérieur.

---

# Shortlist IT à haut ROI — août 2026

```text
0 $      Boomi / Solace / SailPoint Knowledge / QRadar Foundation / New Relic Foundation
0 €      OCI Architect Associate / Microsoft Learn achievements
29 $     Infoblox Associate
50 $     JNCIA / JNCIA-DevOps voucher
50 $     WSO2 API Manager Practitioner
65 $     HPE3
69 $     Infoblox Professional
70,50 $  Terraform Associate
70,50 $  Vault Associate
99 $     GitHub certifications / Infoblox Expert / SUSE AI
100 $    Jamf / CloudStack / IBM AIX / Atlassian Associate
115 $    Dell exam partenaire éligible
125 $    Nokia NSP Automation / NVIDIA NCA-AIIO
126 €    Microsoft role-based
128 €    AWS Associate
149 $    SUSE SLES / RKE2 / Longhorn / Edge / Rancher
150 $    Kafka / MongoDB / Sumo Logic Advanced
175 $    SnowPro Core
199 $    Nutanix NCP
200 $    IBM / Cohesity / NetBackup / NetApp / Pure Associate / Citrix Associate
230 $    Dell Proven Professional retail
249 $    Atlassian Professional
250 $    Omnissa / VMware / CNCF associates / F5
275 $    Kong Gateway / Konnect
295 $    Arista Associate / Specialist / Vault Ops Professional
300 $    MinIO / Cisco DevNet / Citrix Professional / ~ServiceNow CSA
365 €    Ping Identity Professional
390 $    HPE ATP AI Solutions
395 $    Arista Professional
400 $    OpenStack COA / NVIDIA NCP
445 $    CKA / CKS / LFCS / CNPE
450 $    WSO2 Developer → Expert minimal path
```

## Premium / au-dessus du plafond

```text
~530 € HT   Red Hat individual/KIOSK — observation EMEA
899 €       OpenNebula Certified Administrator
1750 € HT   Parallels RAS TPA — exemple France
1995 $      Arista Expert
3440 € HT   Veeam VBR v13 ILT France — seulement un bloc VMCE+
```

---

# Recherche encore active

La couverture IT est désormais large ; les trous restants concernent surtout des **prix non publics** et des programmes émergents :

1. Canonical Academy — tarif checkout France/Europe ;
2. SUSE Virtualization — exam/prix public dès publication ;
3. CyberArk — tarif Defender/Sentry/Guardian ;
4. BeyondTrust — TCO ILT par produit ;
5. Rubrik — RCSA final exam ;
6. Veeam — prix VMCE+/VMCSE France + PRO individuel ;
7. GitLab — prix live des certifications ;
8. Gravitee / Axway — APIM ;
9. AMD ROCm — prix Associate ;
10. Grafana — éventuelle certification proctored ;
11. Lenovo / Supermicro — prix candidat individuel ;
12. Ceph vendor-neutral ;
13. RACF / CICS / IMS ;
14. Ultra Ethernet / OCP / DPU / SuperNIC ;
15. nouveaux credentials SRE / platform engineering / agentic infrastructure.

**Règle : ne jamais transformer un cours, un badge, un prix historique ou une rumeur en certification/prix actuel sans preuve.**
