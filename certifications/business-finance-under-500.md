---
title: "Business, finance, management et compétences professionnelles — sous 500"
type: certification-catalog
tier: under-500
domain:
  - finance-risk
tags:
  - tier/under-500
  - domain/finance-risk
status: active
verified: 2026-08-28
---

# Business, finance, management et compétences professionnelles — sous 500

> Credentials hors IT ou transverses dont le coût d'accès reste sous un budget d'environ 500 € / $.
>
> **Vérification : 28 août 2026**

---

## Vue rapide

| Certification / credential | Domaine | Prix officiel indicatif | Valeur |
|---|---|---:|:---:|
| Entrepreneurship and Small Business — Certiport | Entrepreneuriat | **84 $ US indicatif** | ⭐⭐⭐ |
| PMI Project Management Ready | Projet | **123 $ US indicatif** | ⭐⭐⭐ |
| Six Sigma Yellow Belt — CSSC | Lean / qualité | **99 $** | ⭐⭐⭐ |
| Six Sigma Green Belt — CSSC | Lean / qualité | **195 $** | ⭐⭐⭐⭐ |
| Scrum.org PSM I | Agile / management | **200 $** | ⭐⭐⭐⭐⭐ |
| Scrum.org PSPO I | Produit | **200 $** | ⭐⭐⭐⭐ |
| Scrum.org PAL I | Leadership Agile | **200 $** | ⭐⭐⭐⭐ |
| ISACA COBIT Foundation | Gouvernance | **175 $** | ⭐⭐⭐⭐⭐ |
| ISACA IT Risk Fundamentals | Risque IT | **175 $ membre / 225 $ non-membre** | ⭐⭐⭐⭐ |
| PMI CAPM | Projet | **225 $ membre / 300 $ non-membre** | ⭐⭐⭐⭐ |
| Scrum.org PSM II | Agile | **250 $** | ⭐⭐⭐⭐⭐ |
| CSSC Six Sigma Black Belt | Lean / qualité | **295 $** | ⭐⭐⭐⭐ |
| TOEIC Listening & Reading | Anglais | **~189 € Paris** | ⭐⭐⭐⭐⭐ |
| Cambridge C1 Advanced | Anglais | **~295 € recommandé France** | ⭐⭐⭐⭐⭐ |
| CFA Investment Foundations Certificate | Finance | **350 $** | ⭐⭐⭐⭐ |
| IIBA ECBA | Business Analysis | **195 $ membre / 350 $ non-membre** | ⭐⭐⭐⭐ |
| TOGAF EA Foundation | Architecture d'entreprise | **395 $** | ⭐⭐⭐⭐⭐ |
| PMI-ACP | Agile / projet | **435 $ membre / 495 $ non-membre** | ⭐⭐⭐⭐⭐ |

---

# 1. CFA Institute — Investment Foundations Certificate

**Auto-formation officielle et gratuite :** [CFA Institute Learning Ecosystem](https://www.cfainstitute.org/programs/cfa-program/candidate-resources/curriculum-print-version) — curriculum digital et practice questions inclus dans les frais d'examen.

**Prix standard : 350 $**.

Credential CFA Institute d'introduction au secteur de l'investissement couvrant notamment :

- structure de l'industrie financière ;
- marchés financiers ;
- instruments d'investissement ;
- risque ;
- besoins clients ;
- éthique ;
- rôles dans l'investment management.

Le programme comprend :

- 35 à 65 heures de travail ;
- accès pendant 12 mois ;
- assessment final ;
- certificat partageable.

Les étudiants à temps plein peuvent bénéficier d'un prix spécifique affiché à **79 $**.

Source : https://www.cfainstitute.org/programs/investment-foundations-certificate

**Valeur : ⭐⭐⭐⭐** pour développer une culture finance solide sans entreprendre le CFA Program complet.

---

# 2. PMI — Agile Certified Practitioner

**Auto-formation officielle et gratuite (PMI, toute la gamme ci-dessous) :** [pmi.org/learning/free-online-courses](https://www.pmi.org/learning/free-online-courses) — cours self-paced gratuits (6-10 h), PDU éligibles.

## PMI-ACP

Tarif 2026 :

- membre PMI : **435 $** ;
- non-membre : **495 $**.

Prérequis annoncés :

- diplôme secondaire ;
- 21 heures de formation aux pratiques Agile ;
- expérience projet ;
- expérience Agile selon critères PMI.

Source : https://www.pmi.org/certifications/agile-acp

À 495 $ non-membre, c'est l'une des certifications les plus intéressantes juste sous notre plafond.

**Valeur : ⭐⭐⭐⭐⭐**.

---

# 3. PMI — CAPM

## Certified Associate in Project Management

Tarif de référence :

- membre PMI : **225 $** ;
- non-membre : **300 $**.

Intéressant pour structurer :

- gestion de projet ;
- prédictif ;
- Agile ;
- business analysis ;
- méthodes et vocabulaire PMI.

Source : https://www.pmi.org/certifications/certified-associate-capm

**Valeur : ⭐⭐⭐⭐**.

---

# 4. Scrum.org

Pas de formation obligatoire.

**Auto-formation officielle et gratuite :** [scrum.org/open-assessments](https://www.scrum.org/open-assessments) — assessments gratuits sans certification (Scrum Open, Product Owner Open, Agile Leadership Open…) pour préparer les examens payants.

| Credential | Prix |
|---|---:|
| PSM I | **200 $** |
| PSPO I | **200 $** |
| PAL I | **200 $** |
| PSM II | **250 $** |

Source : https://www.scrum.org/assessments

Pour management / direction :

- **PSM I** : très bon socle ;
- **PAL I** : leadership Agile ;
- **PSM II** : signal plus avancé ;
- **PSPO I** : intéressant pour produit / valeur / priorisation.

---

# 5. Lean Six Sigma — CSSC

Le White Belt est gratuit et classé dans `free-non-it.md`.

Tarifs standards :

| Niveau | Prix |
|---|---:|
| Yellow Belt | **99 $** |
| Green Belt | **195 $** |
| Black Belt | **295 $** |

Une série self-paced complète White → Black est parfois disponible autour de **399 $** selon l'offre en cours.

Source : https://www.sixsigmacouncil.org/six-sigma-certification/

**Valeur :** Green / Black Belt particulièrement utiles pour amélioration continue, opérations et management.

---

# 6. ISACA — gouvernance et risque

## COBIT Foundation

**175 $**.

Credential intéressant pour :

- gouvernance IT ;
- objectifs de gouvernance ;
- alignement métier / SI ;
- contrôle ;
- mesure ;
- responsabilités.

**Valeur : ⭐⭐⭐⭐⭐** pour trajectoire architecture → gouvernance / direction.

## IT Risk Fundamentals

- membre : **175 $** ;
- non-membre : **225 $**.

**Valeur : ⭐⭐⭐⭐**.

Sources : https://www.isaca.org/credentialing

---

# 7. IIBA — ECBA

**Auto-formation officielle et gratuite :** [Global Business Analysis Core Standard](https://www.iiba.org/) — essence du BABOK Guide, gratuite même pour les non-membres ; ECBA Exam Blueprint et sample questions gratuits.

## Entry Certificate in Business Analysis

Tarif :

- membre : **195 $** ;
- non-membre : **350 $**.

Compétences :

- besoins métier ;
- exigences ;
- parties prenantes ;
- analyse ;
- changement ;
- création de valeur.

Source : https://www.iiba.org/knowledgehub/certification/ecba-certification/

**Valeur : ⭐⭐⭐⭐**.

---

# 8. The Open Group — TOGAF EA Foundation

## TOGAF Enterprise Architecture Part 1 / Foundation

**Auto-formation officielle et gratuite :** [opengroup.org](https://www.opengroup.org/togaf) — white papers et standard TOGAF 10th Edition téléchargeables gratuitement (inscription requise).

**395 $**.

Intérêt :

- architecture d'entreprise ;
- ADM ;
- gouvernance d'architecture ;
- principes ;
- artefacts ;
- capacités d'architecture.

Source : https://certification.opengroup.org/examinations

La Foundation seule reste sous le plafond ; le parcours combiné Foundation + Practitioner peut dépasser selon mode d'achat.

**Valeur : ⭐⭐⭐⭐⭐** pour architecture / direction SI.

---

# 9. Anglais

**Auto-formation officielle et gratuite :** voir [`language-certifications.md`](language-certifications.md) — ressources gratuites ETS (TOEIC) et British Council IELTS Ready.

## TOEIC Listening & Reading

Sessions publiques ETS France à Paris observées autour de **189 €** lors de la vérification.

Source : https://www.etsglobal.org/fr/fr/session/search/ile-de-france/paris

## Cambridge C1 Advanced

Prix recommandé France autour de **295 €**, variable selon centre.

Source : https://www.cambridgeenglish.org/exams-and-tests/advanced/

Pour un CV international, un score / niveau externe standardisé est généralement plus fort qu'un badge de cours de langue.

---

# 10. Certiport — entrepreneuriat et projet

Le store US Certiport affiche notamment :

- Entrepreneurship and Small Business : **84 $** ;
- PMI Project Management Ready : **123 $** ;
- Critical Career Skills : **72 $**.

Source : https://store.certiport.com/shop/certiport-products

Ces prix sont **US-only** et servent d'ordre de grandeur ; en France, passer par le provider Certiport local.

---

# 11. Finance — à surveiller

## CFA Institute Principles / Sustainable Investing

Le store CFA affiche actuellement certains produits `Principles of Sustainable Investing` en chinois avec une structure de prix inhabituelle : inscription / curriculum facturés séparément et certaines lignes de certificate à 0 $. Cette offre ne doit pas être présentée comme une certification gratuite générale sans vérifier le parcours exact, la langue et le coût total.

**Statut : watchlist.**

## CFA Program

À partir de 2026 :

- Level I early bird : **1 140 $** ;
- Level I standard : **1 490 $**.

Le CFA Program ira donc dans la future tranche `>500`.

---

# Ordre intéressant pour un profil technique évoluant vers le management

Sans chercher à collectionner les badges :

1. **TOEIC / Cambridge C1** — preuve d'anglais ;
2. **COBIT Foundation — 175 $** — gouvernance ;
3. **PSM I — 200 $** ou PAL I — leadership Agile ;
4. **Six Sigma Green Belt — 195 $** — amélioration continue ;
5. **CAPM** si besoin de formaliser la gestion de projet ;
6. **TOGAF Foundation — 395 $** — architecture d'entreprise ;
7. **PMI-ACP — jusqu'à 495 $** si les prérequis sont remplis ;
8. **CFA Investment Foundations — 350 $** si l'objectif est d'ajouter une vraie culture finance.
