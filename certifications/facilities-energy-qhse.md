---
title: "Facility management, énergie & QHSE"
type: certification-catalog
tier: general
domain:
  - datacenter-facilities
tags:
  - tier/general
  - domain/datacenter-facilities
status: active
verified: 2026-08-28
---

# Facility management, énergie & QHSE

> Credentials de facility management, energy management, mesure de performance énergétique et santé-sécurité. Vérifié fin août 2026.

---

# 1. IFMA — Certified Facility Manager

**Auto-formation officielle :** [ifma.org/professional-development](https://www.ifma.org/professional-development/online-learning/) — 55+ cours self-paced payants, dont le parcours *Essentials of Facility Management*.

## CFM

IFMA affiche en 2026 :

- **615 $** pour un Full Member ;
- **910 $** tarif standard.

### Point important

Aucune formation IFMA n'est obligatoire pour s'inscrire à l'examen si le candidat satisfait les critères d'éligibilité.

Le CFM est donc une vraie certification exam-based où les ressources de préparation sont facultatives.

### Format

- 120 questions ;
- 3 heures ;
- 10 domaines FM ;
- exigences d'expérience / formation selon voie d'éligibilité.

### Recertification

Validité : **3 ans**.

- membre : **299 $** ;
- non-membre : **379 $** ;
- continuing education nécessaire.

Source :

- https://www.ifma.org/cfm

---

# 2. IFMA — Facility Management Professional

## FMP

Le FMP est un credential knowledge-based basé sur quatre modules obligatoires et leurs assessments.

## Bundle 2026

- Full Member : **1 850 $** ;
- standard : **2 150 $**.

Les quatre domaines comprennent notamment :

- Operations & Maintenance ;
- Project Management ;
- Finance & Business ;
- Leadership & Strategy.

Les modules individuels commencent autour de :

- **550 $ membre** ;
- **680 $ standard**.

Source :

- https://www.ifma.org/fmp/

---

# 3. Association of Energy Engineers — attention au TCO

Les frais AEE d'application + examen semblent modérés, mais **la plupart des certifications exigent la participation à un approved training program**.

Il ne faut donc pas les classer sur le seul prix du voucher.

**Auto-formation officielle et gratuite (partielle) :** [Schneider Electric University](https://www.se.com/ww/en/about-us/university/) propose des modules gratuits reconnus par certains partenaires AEE pour la préparation Professional Energy Manager (PEM).

---

## Certified Energy Manager — CEM

### Frais certification/examen US

**500 $**.

- retest : **250 $** ;
- renewal : **300 $ tous les 3 ans**.

### Formation obligatoire

AEE indique que tous les candidats doivent suivre un approved training program.

Exemples 2026 US / online :

- Accelerated CEM : environ **1 300–1 500 $** selon membership / date ;
- Premier CEM : environ **2 100–2 445 $** selon membership / date.

Le coût exact international passe par un Approved Training Partner local.

Sources :

- https://www.aeecenter.org/certified-energy-manager/becoming-a-cem/
- https://education.aeecenter.org/2026_Training_Programs

### TCO minimal réaliste US

Selon programme :

```text
training     ~1 300–2 445 $
cert/exam       500 $
-------------------------
~1 800–2 945 $
```

avant éventuel membership.

---

## Certified Energy Auditor — CEA

- application + exam : **500 $** ;
- retest : **250 $** ;
- renewal : **300 $ / 3 ans** ;
- **approved training obligatoire**.

Exemple formation 2026 : environ **2 100–2 445 $** selon tarif.

Source :

- https://www.aeecenter.org/certified-energy-auditor/becoming-a-cea/

---

## Certified Measurement & Verification Professional — CMVP

### Certification/exam US

- **400 $** ;
- retest : **200 $** ;
- renewal : **300 $ / 3 ans**.

### Formation

Programme 2026 :

- environ **1 900–2 100 $**.

Sources :

- https://www.aeecenter.org/certified-measurement-verification-professional/becoming-a-cmvp/
- https://education.aeecenter.org/products/certified-measurement-verification-professional-cmvp-training-program-october-2026

### TCO

Environ **2 300–2 500 $** selon tarif de formation.

---

# 4. Autres AEE

## Certified Sustainable Development Professional — CSDP

- certification/exam : **400 $** ;
- retest : **200 $** ;
- renewal : **300 $ / 3 ans** ;
- formation approved requise.

Exemples formation 2026 : environ **1 200–1 400 $**.

Source :

- https://www.aeecenter.org/aee-certifications/certified-sustainable-development-professional/becoming-a-csdp/

## Energy Efficiency Practitioner — EEP

- certification/exam : **400 $** ;
- retest : **200 $** ;
- renewal : **300 $ / 3 ans** ;
- formation requise.

## Certified Renewable Energy Professional — REP

- certification/exam : **400 $** ;
- retest : **200 $** ;
- renewal : **300 $ / 3 ans** ;
- formation requise.

## Certified Carbon Auditing Professional — CAP

- certification/exam : **400 $** ;
- retest : **200 $** ;
- renewal : **300 $ / 3 ans** ;
- formation requise.

## Certified Energy Procurement Professional — CEP

- certification/exam : **400 $** ;
- retest : **200 $** ;
- renewal : **300 $ / 3 ans** ;
- formation requise.

Sources :

- https://www.aeecenter.org/

---

# 5. IOSH Level 3 Certificate

**Portail officiel :** [iosh.com/qualifications-and-courses](https://iosh.com/qualifications-and-courses) — dispensé via des organismes de formation agréés IOSH (pas de cours gratuit identifié).

## IOSH Level 3 Certificate in Occupational Safety and Health Principles and Practice

La grille Awarding Organisation 2026–2027 affiche :

**180 £**

comprenant :

- learner registration ;
- un examen OSH Principles ;
- un examen OSH Practice scenario-based ;
- e-certificate.

Resit : **70 £ par examen**.

### Important

Ces frais sont ceux de l'Awarding Organisation : les coûts d'enseignement d'un centre / provider peuvent s'ajouter.

Source :

- https://iosh.com/qualifications

---

# 6. IOSH Level 6 Diploma

La même grille AO affiche :

**295 £**

pour :

- learner registration ;
- première soumission des units 1, 2 et 3 ;
- e-certificate.

Resubmission : **85 £ par unit**.

Là encore, la tuition / formation du study centre doit être ajoutée au TCO.

---

# 7. ISO 45001 Lead Auditor — BSI France

BSI France affiche pour le **CQI/IRCA ISO 45001 Lead Auditor** :

- 5 jours ;
- virtual classroom ;
- **2 495 € + TVA** sur la page live consultée ;
- examen / skills assessment intégrés au parcours.

Source :

- https://www.bsigroup.com/fr-FR/training-courses/iso-45001-lead-auditor-cqi-irca-international-training/

Une ancienne grille PDF BSI France affichait environ 3 020 €, ce qui illustre l'importance d'utiliser le prix live de la session plutôt qu'un ancien catalogue.

---

# Vue rapide

| Credential | Coût initial indicatif |
|---|---:|
| IOSH Level 3 AO fees | **180 £ + tuition provider** |
| IOSH Level 6 AO fees | **295 £ + tuition provider** |
| AEE CEM exam/application | **500 $ + mandatory training** |
| AEE CEA exam/application | **500 $ + mandatory training** |
| AEE CMVP | **400 $ + mandatory training** |
| IFMA CFM membre | **615 $** |
| IFMA CFM standard | **910 $** |
| IFMA FMP member bundle | **1 850 $** |
| IFMA FMP standard bundle | **2 150 $** |
| BSI ISO 45001 Lead Auditor | **2 495 € + TVA** |
| AEE CEM from-zero | **~1,8–2,95 k$** selon training |
| AEE CMVP from-zero | **~2,3–2,5 k$** |

---

# À poursuivre

- NEBOSH General Certificate / Diploma par provider France ;
- IOSH Chartered membership route ;
- IFMA SFP ;
- IWFM ;
- RICS FM / built environment ;
- AEE France / Approved Training Partner exact ;
- ISO 50001 Lead Energy Manager ;
- CMVP Europe ;
- building commissioning ;
- HVAC / ASHRAE certifications ;
- fire protection credentials ;
- NFPA certifications ;
- electrical safety ;
- critical facilities operations.
